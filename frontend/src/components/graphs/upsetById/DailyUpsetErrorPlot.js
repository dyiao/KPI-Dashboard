...
const DailyUpsetPlot = ({ flag }) => {
    ...
    const { isLoading, isError, data, error, refetch, isFetching } = useQuery("singleUpsetErrorDailyStats", () => axios({
    ...
    useEffect(() => {
        if (isFetching) {
            queryClient.cancelQueries("singleUpsetErrorDailyStats")
        }
        if (endDate) {
            refetch()
        }
